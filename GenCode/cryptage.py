...
def encrypt_file(text, file_name):
    ...
    with open(os.path.join(os.path.dirname(__file__), file_name), "ab") as f_out:
        # Si le fichier de sortie n'est pas vide, ajoute un délimiteur (une nouvelle ligne) avant d'écrire le texte chiffré
        if os.path.getsize(os.path.join(os.path.dirname(__file__), file_name)) != 0:
            f_out.write(b"\n")
...
def decrypt_file(encrypted_file_name, decrypted_file_name):
    ...
    with open(os.path.join(os.path.dirname(__file__), decrypted_file_name), "a") as f_out:
